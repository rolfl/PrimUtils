 ...
 * <p>
 * This class is designed to be extended with additional functionality building
 * on the key/index system. As a result, most methods are protected, and need to
 * be exposed or used by the implementing instance. To avoid name problems, most
 * methods have the name prefix "ki" for "key-index". Additionally, these
 * methods are final, and cannot be changed/overridden
 * <p>
 ...
 * values returned by the {@link #kiAdd(int)} method is not. The system has the
 ...
 *
 */
public abstract class AbstractIntKeyIndex {
     ...
     */
    protected AbstractIntKeyIndex(final int capacity) {
     ...
     */
    protected final boolean kiIsEmpty() {
        return kiSize() == 0;
    }
     ...
     */
    protected final boolean kiContainsKey(final int key) {
        return kiGetIndex(key) >= 0;
    }
     ...
     * @return true if the key was previously mapped.
     */
    protected final boolean kiContainsIndex(final int index) {
     ...
     */
    protected final int kiAdd(final int key) {
     ...
     *         mapped.
     */
    protected final int kiGetIndex(final int key) {
        final int bucket = bucketId(key);
     ...
     */
    protected final int kiGetKey(final int index, final int notThere) {
        return kiContainsIndex(index) ? getValue(keys, index) : notThere;
    }
     ...
     *         not mapped.
     */
    protected final int kiRemove(final int key) {
        final int bucket = bucketId(key);
        final int pos = locate(bucketData[bucket], bucketSize[bucket], key);
        if (pos < 0) {
            return -1;
     ...
     */
    protected final int[] kiGetKeys() {
        return kiStreamKeys().toArray();
    }
     ...
     *         {@link #kiGetKeys()}
     */
    protected final int[] kiGetIndices() {
        return kiStreamIndices().toArray();
    }
     ...
     * @return the mapped keys.
     */
    protected final IntStream kiStreamKeys() {
     ...
     *         {@link #kiGetKeys()}
     */
    protected final IntStream kiStreamIndices() {
        return liveIndices();
    }
     ...
     */
    protected final Stream<IntKIntVEntry> kiStreamEntries() {
        return liveIndices().mapToObj(i -> new IntKIntVEntry(getValue(keys, i), i));
    }
     ...
     */
    protected final String kiReport() {
        ...
        return String.format("IntIntMap size %s (used %d, deleted %d) buckets %d hashspace %d longest %d valspace %d",
                kiSize(), size, deletedCount, bucketSize.length, allocated, max, vals);
    }
     ...
     */
    protected final int kiKeyHashCode() {
        if (kiSize() == 0) {
            return 0;
        }
        return liveIndices().map(i -> getValue(keys, i)).map(k -> Integer.rotateLeft(k, k)).reduce((x, p) -> x ^ p)
                .getAsInt();
    }
     ...
     * @return a useful hash of just the keys in this map.
     */
    protected final int kiIndexHashCode() {
        if (kiSize() == 0) {
            return 0;
        }
     ...
     * @return true if this instance has the exact same key/index mappings.
     */
    protected final boolean kiEquals(AbstractIntKeyIndex other) {
        if (other == null) {
            return false;
        }
        if (other == this) {
    ...
    private boolean same(final AbstractIntKeyIndex them, final int index) {
        final int k = getValue(keys, index);
        int t = them.kiGetIndex(k);
        if (t != index) {
            return false;
        }
        return true;
    ...
    private void deleteIndex(final int index) {
        ...
        setValue(keys, index, -1); // make the delete visible in the keys.
    }
    ...
    private int addKeyValue(final int key) {
        ...
        final int row = getMatrixRow(size);
        final int col = getMatrixColumn(size);

        if (keys.length == row) {
            keys = Arrays.copyOf(keys, extendSize(row));
        }
        if (keys[row] == null) {
            keys[row] = buildIntRow();
        }
        keys[row][col] = key;
        return size++;
    }
    ...
    private IntStream liveIndices() {
        return StreamSupport.intStream(new IndexSpliterator(modCount, kiSize(), 0, bucketData.length), false);
    }
